...
class ContextProcessor:
    ...
    def _determine_relevant_file_types(self, query: str, task_type: str) -> List[str]:
        ...
        # Base file types for different task types
        type_mapping = {
            'development': ['.py', '.js', '.ts', '.java', '.cpp', '.c', '.go', '.rs'],
            'documentation': ['.md', '.rst', '.txt', '.doc'],
            'configuration': ['.json', '.yaml', '.yml', '.toml', '.ini', '.cfg'],
            'testing': ['.py', '.js', '.ts', '.test.js', '.spec.js'],
            'design': ['.css', '.scss', '.less', '.html'],
            'bug fix': ['.py', '.js', '.ts', '.log']
        }
        
        relevant_types = type_mapping.get(task_type.lower(), ['.py', '.js', '.md'])
