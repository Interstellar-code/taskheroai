        ...
        try:
            async with self.session.get(f"{self.base_url}/api/version") as response:
                if response.status == 200:
                    ...
                    raise ProviderError(f"Ollama server returned status {response.status}")
    ...
    async def _ensure_model_available(self) -> None:
        ...
        try:
            ...
            async with self.session.get(f"{self.base_url}/api/tags") as response:
                if response.status != 200:
                    raise ProviderError(f"Failed to list Ollama models: {response.status}")
        ...
        try:
            if streaming:
                ...
                async with self.session.post(
                    ...
                    if response.status != 200:
                        raise ProviderError(f"Ollama request failed with status {response.status}")
    ...
    async def stream_response(
        ...
        try:
            ...
            async with self.session.post(
                ...
                if response.status != 200:
                    raise ProviderError(f"Ollama streaming request failed with status {response.status}")
