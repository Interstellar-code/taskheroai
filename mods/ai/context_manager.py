...
class CodebaseContextManager:
    ...
    async def _generate_project_summary(self) -> str:
        ...
        try:
            ...
            if file_types:
                ...
                summary += ", ".join(type_strs[:5])  # Show top 5 types
    ...
    async def _generate_file_structure(self) -> str:
        ...
        try:
            ...
            dir_files = {}
            for file_path in indexed_files[:20]:  # Limit to prevent huge structures
                if root_path:
                ...
                if dir_name not in dir_files:
                    dir_files[dir_name] = []
                dir_files[dir_name].append(os.path.basename(rel_path))
            
            # Format structure
            for dir_name, files in sorted(dir_files.items()):
                structure_lines.append(f"{dir_name}/")
                for filename in sorted(files[:5]):  # Limit files per directory
                    structure_lines.append(f"  {filename}")
                if len(files) > 5:
                    structure_lines.append(f"  ... and {len(files) - 5} more files")
