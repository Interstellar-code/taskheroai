...
class VersionManager:
    ...
    def __init__(self, repository_url: str = "https://github.com/Interstellar-code/taskheroai", settings_manager=None):
        """
        Initialize the version manager.

        Args:
            repository_url: GitHub repository URL
            settings_manager: Settings manager for consolidated cache storage
        """
    ...
    def _get_cached_remote_version(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Try to get from consolidated settings first
            if self.settings_manager:
                settings = self.settings_manager.get_settings()
                git_settings = settings.get("git", {})
                cached_data = git_settings.get("version_cache")

            # Fallback to old cache file if not in settings
            if not cached_data:
    ...
    def _cache_remote_version(self, version_data: Dict[str, Any]) -> None:
        """Cache remote version data in consolidated settings."""
        try:
            ...
            if self.settings_manager:
                ...
                self.logger.debug("Cached remote version data in file")
        except Exception as e:
    ...
    def clear_cache(self) -> None:
        ...
        try:
            ...
            if cache_file.exists():
                cache_file.unlink()
                self.logger.info("Old version cache file cleared")
        except Exception as e:
