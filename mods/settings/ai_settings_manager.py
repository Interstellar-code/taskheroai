...
class AISettingsManager(BaseManager):
    ...
    def __init__(self, env_manager: Optional[EnvironmentManager] = None):
        ...
        self.fallback_config = {
            'enabled': True,
            'chain': ['openai', 'anthropic', 'openrouter', 'deepseek', 'ollama'],
            'retry_attempts': 3,
            'timeout_seconds': 30,
            'auto_switch': True
        }
    ...
    def get_fallback_settings(self) -> Dict[str, Any]:
        """
        Get fallback AI provider settings.

        Returns:
            Dictionary of fallback settings
        """
        try:
            # Get from environment variables
            fallback_chain = self.env_manager.get_env_var('AI_PROVIDER_FALLBACK_CHAIN', '')
            if fallback_chain:
                chain = [provider.strip() for provider in fallback_chain.split(',')]
            else:
                chain = self.fallback_config['chain']

            return {
                'enabled': self.env_manager.get_env_var('AI_FALLBACK_ENABLED', 'true').lower() == 'true',
                'chain': chain,
                'retry_attempts': int(self.env_manager.get_env_var('AI_FALLBACK_RETRY_ATTEMPTS', '3')),
                'timeout_seconds': int(self.env_manager.get_env_var('AI_FALLBACK_TIMEOUT', '30')),
                'auto_switch': self.env_manager.get_env_var('AI_FALLBACK_AUTO_SWITCH', 'true').lower() == 'true'
            }

        except Exception as e:
            self.logger.error(f"Error getting fallback settings: {e}")
            return self.fallback_config.copy()

    def set_fallback_settings(self, settings: Dict[str, Any]) -> bool:
        """
        Set fallback AI provider settings.

        Args:
            settings: Dictionary of fallback settings
        ...
        try:
            # Validate settings
            required_keys = ['enabled', 'chain', 'retry_attempts', 'timeout_seconds', 'auto_switch']
            for key in required_keys:
                if key not in settings:
                    self.logger.error(f"Missing required fallback setting: {key}")
                    return False

            # Set environment variables
            env_vars = {
                'AI_FALLBACK_ENABLED': str(settings['enabled']).lower(),
                'AI_PROVIDER_FALLBACK_CHAIN': ','.join(settings['chain']),
                'AI_FALLBACK_RETRY_ATTEMPTS': str(settings['retry_attempts']),
                'AI_FALLBACK_TIMEOUT': str(settings['timeout_seconds']),
                'AI_FALLBACK_AUTO_SWITCH': str(settings['auto_switch']).lower()
            }

            for key, value in env_vars.items():
                self.env_manager.set_env_var(key, value)

            # Update local config
            self.fallback_config.update(settings)

            self.logger.info("Fallback settings updated successfully")
            return True

        except Exception as e:
            self.logger.error(f"Error setting fallback settings: {e}")
            return False
    ...
    def get_fallback_chain(self) -> List[str]:
        ...
        """
        settings = self.get_fallback_settings()
        return settings.get('chain', self.fallback_config['chain'])
    ...
    def set_fallback_chain(self, chain: List[str]) -> bool:
        ...
        Args:
            chain: List of provider names in fallback order
        ...
        try:
            # Validate providers
            valid_providers = ['openai', 'anthropic', 'ollama', 'openrouter', 'deepseek']
            for provider in chain:
                if provider not in valid_providers:
                    self.logger.error(f"Invalid provider in fallback chain: {provider}")
                    return False

            # Update settings
            current_settings = self.get_fallback_settings()
            current_settings['chain'] = chain
            return self.set_fallback_settings(current_settings)

        except Exception as e:
            self.logger.error(f"Error setting fallback chain: {e}")
            return False
