...
def test_about_generation_with_provider(provider: str, model: str, config: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
    ...
    try:
        ...
        async def custom_content_generation(project_info):
            """Enhanced content generation with specific provider/model using new enhanced methods."""
            try:
                print(f"🤖 Using AI: {provider} ({model}) for enhanced dynamic content generation")
                ...
                print("📋 Section 1: Why TaskHero AI Exists & Vision Statement generation (Enhanced)...")
                basic_info = await about_manager._generate_basic_info_enhanced(enhanced_context, provider, model)

                print("🔍 Section 3: Problems Solved generation (Enhanced)...")
                problems_solved = await about_manager._generate_problems_solved_enhanced(enhanced_context, provider, model)

                print("📊 Section 4: Solution Flow Diagram generation (Enhanced)...")
                solution_flow = await about_manager._generate_solution_flow_enhanced(enhanced_context, provider, model)

                print("⚙️ Section 5: How TaskHero AI Works generation (Enhanced)...")
                how_it_works = await about_manager._generate_how_it_works_enhanced(enhanced_context, provider, model)

                print("🎨 Section 6: User Experience Goals generation (Enhanced)...")
                ux_goals = await about_manager._generate_ux_goals_enhanced(enhanced_context, provider, model)

                print("👥 Section 7: Target Users generation (Enhanced)...")
                user_personas = await about_manager._generate_user_personas_enhanced(enhanced_context, provider, model)

                print("🚶‍♂️ Section 8: Key User Journeys generation (Enhanced)...")
                user_journeys = await about_manager._generate_user_journeys_enhanced(enhanced_context, provider, model)

                print("📊 Section 9: Success Metrics generation (Enhanced)...")
                success_metrics = await about_manager._generate_success_metrics_enhanced(enhanced_context, provider, model)

                print("🎯 Section 10: Current Product Focus generation (Enhanced)...")
                current_focus = await about_manager._generate_current_focus_enhanced(enhanced_context, provider, model)

                print("🚀 Section 11: Recent Improvements generation (Enhanced)...")
                recent_improvements = await about_manager._generate_recent_improvements_enhanced(enhanced_context, provider, model)

                print("📈 Section 12: Future Roadmap generation (Enhanced)...")
                future_roadmap = await about_manager._generate_future_roadmap_enhanced(enhanced_context, provider, model)

                # Combine all generated content with enhanced metadata
                dynamic_context = {
                    **basic_info,
                    ...
                    'improvements_summary': f'Enhanced AI-generated improvements using {provider}/{model} with {context_quality_score} context sources and provider-specific optimization.',
                    'generation_metadata': {
                        'provider': provider,
                        'model': model,
                        'context_sources': context_quality_score,
                        'enhanced_features_count': len(enhanced_features),
                        'capabilities_count': len(project_capabilities),
                        'ai_providers_integrated': len(ai_integration_details.get('providers', [])),
                        'workflow_types': len(task_management_context.get('workflow_types', []))
                    }
                }

                print("✅ All enhanced about document sections generated successfully!")
                print(f"📊 Generation metadata: {context_quality_score} context sources, {len(enhanced_features)} features, {len(project_capabilities)} capabilities")
